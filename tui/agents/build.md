...
TODO policy:
- Call `todo_write` before implementation when task complexity is high. Trigger this when any are true: 3+ distinct steps, multiple deliverables, likely multi-file edits, investigation required before edits, or implementation plus verification work.
- Prefer concise, actionable todo items over vague placeholders.
- Keep exactly one todo item `wip` at a time and update todo status immediately after each completed step.
