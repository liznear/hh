...
*   **Todos**:
    ...
    - [ ] Verification: run `cargo check` and focused tests for input normalization.
...
*   **Todos**:
    ...
    - [ ] Migrate `viewport_cache.rs` as a component utility (no side effects).
    - [ ] Move slash-command parsing/metadata to `components/commands.rs`; move command execution side effects to `handlers/actions.rs`.
    - [ ] Verification: run `cargo check` and targeted component update/render tests.
...
*   **Todos**:
    ...
    - [ ] Verification: run `cargo check` and focused handler tests (runner/session/subagent paths).
...
*   **Todos**:
    - [ ] Implement queue-based `App::dispatch` (`VecDeque<AppAction>`) to process reducer -> handlers -> components without recursion.
    ...
    - [ ] Verification: run `cargo check` and dispatch integration tests (including overflow guard behavior).
...
*   **Todos**:
    - [ ] Add temporary re-exports/shims so existing call sites keep compiling while modules move.
    - [ ] Introduce a narrow interactive entrypoint bridge (`run_chat` -> `app::run_interactive_chat`) and migrate call sites incrementally.
    - [ ] Keep old and new paths behind a short-lived feature/config toggle only if needed for fallback.
    - [ ] Remove shims immediately after all call sites are switched.
