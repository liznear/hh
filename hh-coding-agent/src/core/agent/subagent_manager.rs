...
impl SessionSink for std::sync::Arc<dyn SessionSink + Send + Sync> {
...
use crate::session::{SessionEvent, SubAgentFailureReason, SubAgentLifecycleStatus, event_id};
...
impl SubagentManager {
    ...
    async fn mark_running(&self, parent_session: &dyn SessionSink, task_id: &str) {
        let mut state = self.inner.lock().await;
    ...
    async fn finish_task(
        &self,
        parent_session: &dyn SessionSink,
        task_id: &str,
...
/// A null session sink that does nothing (used for error cases).
struct NullSessionSink;

impl SessionSink for NullSessionSink {
    fn append(&self, _event: &SessionEvent) -> anyhow::Result<()> {
        Ok(())
    }
}
...
mod tests {
    ...
    async fn start_and_finish_persist_expected_session_lifecycle_events() {
        let sink = Arc::new(InMemorySessionSink {
    ...
    async fn rejects_requests_exceeding_depth_limit() {
        ...
        let err = manager
            .start_or_resume(request, sink.clone() as Arc<dyn SessionSink + Send + Sync>)
            .await
