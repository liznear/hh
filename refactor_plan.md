...
- **Tasks:**
  - Finalize `RunnerState` (initially `todo_items`, `context_tokens`, plus future typed fields).
  - Finalize `CoreInput` and `CoreOutput` enums (transport-safe error payloads).
  - Finalize `RunnerInput` and `RunnerOutput` enums, including cancellation path.
  - Document protocol invariants: one tool result per `call_id`, and next provider call only after all tool calls in the current turn resolve.
...
- **Tasks:**
  - Introduce a typed `StatePatch`/`StateOp` model.
  - Update `Tool::execute` and `ToolExecutor::execute` to return `ToolResult` + `StatePatch`.
  - Keep patch generation inside tools; keep patch application centralized in the Runner.
  - Implement `TodoWriteTool` to emit a patch that updates canonical TODO state.
...
- **Tasks:**
  ...
  - Persist a serialized `RunnerState` snapshot in session metadata.
